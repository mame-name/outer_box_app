...
# エリアの「肉付け」設定
# ==========================================
Y_MARGIN_PERCENT = 0.1  # 入数に対して上下10%の範囲を「適正エリア」とする
# ==========================================
                ...
                if not plot_data.empty:
                    for box_type in selected_boxes:
                        ...
                        if plot_mode == "実績を囲む（エリア）":
                            # --- 実績をベースに上下にマージンを持たせた帯(バンド)を作成 ---
                            # 代表的なx(体積)におけるy(入数)の平均をとる
                            band_base = group.groupby("単一体積")["入数"].mean().reset_index()
                            x_vals = band_base["単一体積"].tolist()
                            y_vals = band_base["入数"].tolist()

                            # 上端(y + 10%) と 下端(y - 10%)
                            y_upper = [y * (1 + Y_MARGIN_PERCENT) for y in y_vals]
                            y_lower = [y * (1 - Y_MARGIN_PERCENT) for y in y_vals]

                            # 塗りつぶし用パス（往路：上端、復路：下端）
                            x_path = x_vals + x_vals[::-1]
                            y_path = y_upper + y_lower[::-1]

                            fig.add_trace(go.Scatter(
                                x=x_path, y=y_path,
                                fill='toself',
                                fillcolor=color_map[box_type],
                                line=dict(color='rgba(255,255,255,0)'), # 外枠は消す
                                opacity=0.3,
                                name=f"{box_type} 適正域",
                                hoverinfo='skip'
                            ))
                            ...
                            fig.add_trace(go.Scatter(
                                x=x_vals, y=y_vals,
                                mode='lines',
                                line=dict(color=color_map[box_type], width=1, dash='dot'),
                                showlegend=False
                            ))
                        else:
                            # 通常のプロットモード
