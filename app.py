    ...
    if uploaded_file:
        try:
            ...
            if not df_display.empty:
                ...
                # 数値変換の念押し（グラフ描画エラー防止）
                df_display["重量（箱）"] = pd.to_numeric(df_display["重量（箱）"], errors='coerce')
                df_display["比重"] = pd.to_numeric(df_display["比重"], errors='coerce')
                plot_data = df_display.dropna(subset=["重量（箱）", "比重"])
                ...
                fig.update_layout(legend_title_text='外箱種類', margin=dict(l=0, r=0, t=30, b=0))
