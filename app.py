    ...
    if uploaded_file:
        try:
            ...
            if not df_display.empty:
                ...
                plot_data = df_display[df_display["体積"] > 0].copy()
                
                # ベースの散布図
                fig = px.scatter(
                    plot_data, x="体積", y="入数", color="外箱",
                    hover_name="製品名",
                    hover_data={"製品コード":True, "体積":":.1f", "入数":True, "外箱":True, "製品サイズ":True},
                    template="plotly_white", height=650,
                    labels={"体積": "推定合計体積 (重量/比重*入数)", "入数": "入数 [個]"}
                )

                # 手書き図を再現：外箱ごとの「領域」を塗りつぶし
                for box_type in plot_data["外箱"].unique():
                    group = plot_data[plot_data["外箱"] == box_type]
                    if len(group) >= 3:
                        # 簡易的な領域表示（データの外周をうっすら繋ぐ）
                        fig.add_trace(go.Scatter(
                            x=group["体積"], y=group["入_数"] if "入_数" in group else group["入数"],
                            fill='toself',
                            fillcolor='rgba(150, 150, 150, 0.1)', # 重なりが見える半透明グレー
                ...
                if i_weight and i_sg and i_pcs:
                    try:
                        sim_vol = float(i_weight) / float(i_sg) * float(i_pcs)
                        sim_pcs = float(i_pcs)
                        fig.add_trace(go.Scatter(
                            x=[sim_vol], y=[sim_pcs],
                            mode='markers',
